...
LISTS = ['./list/test.list', './list/train.list']
...
def get_image_batch_for_segment(video_path, frames_list, num_frames_per_clip=NUM_FRAMES_PER_CLIP, \
    ...
    for clip_index in range(0, num_clips):
        ...
        for frame_index in range(start_frame_index, start_frame_index + num_frames_per_clip):
            ...
            clip.append(np.expand_dims(image_cropped, axis=0)) # (H, W, C) -> (1, H, W, C)
...
def save_video_features(filename, num_segments_per_video=NUM_SEGMENTS_PER_VIDEO):
    lines = open(filename, 'r').readlines()
    for video_index in tqdm(range(len(lines))):
        line = lines[video_index].strip('\n').split()
        dirname = line[0]
        resized_dirname = dirname + '_resized'
        for video, _, frames in os.walk(resized_dirname):
            ...
            with h5py.File(FEATURE_FILE, 'w') as hf:
                hf.create_dataset(resized_dirname, data=video_features)
            break

if __name__ = '__main__':
    for listname in LISTS:
        save_video_features(listname)
